...
const ExChangeBooks = () => {
    ...
    const { exchangeBooks, loading, error, filteredBooks, resultPerPage, totalBooks } = useSelector(state => state.exchangeBooks);
    ...
    useEffect(() => {
        ...
        dispatch(getAllExchangeBooks(category, currentPage))

    }, [currentPage, category, dispatch, error]);


    const searchedBooks = exchangeBooks.filter((book) => {
